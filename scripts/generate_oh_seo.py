#!/usr/bin/env python3
"""
Generate Ohio SEO pages: state page, all county pages, and 50K+ city pages.
...
CITY_POP = {
    "Columbus": 933263, "Cleveland": 365379, "Cincinnati": 314915, "Toledo": 265638, "Akron": 189664, "Dayton": 136346, "Parma": 79350, "Canton": 69211, "Lorain": 65751, "Hamilton": 63953, "Youngstown": 59000, "Springfield": 58000, "Kettering": 57000, "Elyria": 53000, "Middletown": 51500, "Newark": 51000,
}
...
def llm(text_prompt):
    ...
    payload = {"model": LLM_MODEL, "messages": [
        {"role": "system", "content": "You are a local writer crafting natural website copy about Ohio locations. No markdown, no em dashes, no asterisks. Plain paragraphs only. Sound like a person."},
...
def content_paragraphs(name, kind):
    if kind == "city":
        prompt = f"Write 2 short paragraphs about {name}, Ohio. P1: what it's known for. P2: what renters should know about the local market and housing help. Return 2 paragraphs separated by a blank line."
    elif kind == "county":
        prompt = f"Write 2 short paragraphs about {name} County, Ohio. P1: what makes it distinct. P2: what a renter should know about rental housing help and the court system. Return 2 paragraphs separated by a blank line."
    else:
        prompt = f"Write 3 short paragraphs about Ohio for a tenant-eviction self-help website. P1: notable fact. P2: rental housing context. P3: practical eviction-process tip. Return 3 paragraphs separated by blank lines."
    t = llm(prompt)
    if not t:
        return [f"{name} is a community in Ohio."] * (3 if kind == "state" else 2)
...
<section class="section"><div class="container"><div class="section-intro"><span class="eyebrow">Statewide starting points</span><h2>Resources that may serve {name} County</h2><p>These statewide or regional portals are starting points, not guarantees of funding or representation.</p></div><div class="resource-list"><article class="resource"><span class="type">Referral</span><h3><a href="https://www.211.org/" rel="nofollow noopener" target="_blank">2-1-1 Ohio ↗</a></h3><p>Rent, utilities, shelter, food and crisis referrals. Dial 211.</p></article><article class="resource"><span class="type">Tenant Help</span><h3><a href="https://motenanthelp.org/" rel="nofollow noopener" target="_blank">Ohio Tenant Help ↗</a></h3><p>Free tenant resource and eviction document engine.</p></article><article class="resource"><span class="type">Legal Aid</span><h3><a href="https://www.lsmo.org/" rel="nofollow noopener" target="_blank">Legal Services of Ohio ↗</a></h3><p>Statewide legal-aid network for low-income tenants.</p></article></div></div></section>
...
def build_city_page(cname, ccounty):
    ...
    head = build_head(
        f"Eviction Help in {cname}, Ohio | evictions.help",
        ...
        f'<li><a href="/">Home</a></li><li><a href="/{STATE_SLUG}/">Ohio</a></li><li><a href="/{STATE_SLUG}/{county_slug}/">{ccounty} County</a></li><li>{cname}</li>',
        f'data-state="OH" data-county="{ccounty} County" data-city="{cname}"'
    )
    hero = build_hero(
...
def main():
    ...
    # 1. State page
    print("Generating Ohio state page...", flush=True)
    paras = content_paragraphs("Ohio", "state")
    content = "\n".join(f"<p>{p}</p>" for p in paras)
    ...
    state_html = build_head(
        "Eviction Help in Ohio | evictions.help",
        "Check eligibility for eviction self-help document preparation across Ohio counties and cities.",
        f"https://evictions.help/{STATE_SLUG}/",
        '<li><a href="/">Home</a></li><li>Ohio</li>',
        'data-state="OH" data-county="" data-city=""'
    ) + build_hero(
        "Ohio eviction document help",
        "Eviction paperwork help for tenants in Ohio",
        "Start with eight questions. Your Ohio location is carried into the guided intake and $299 document packet workflow.",
        'data-state="OH" data-county="" data-city=""'
    ) + f'''<section class="section"><div class="container">{content}</div></section>
<section class="section section-soft"><div class="container"><div class="section-intro"><span class="eyebrow">Counties</span><h2>Ohio county directories</h2></div><div class="location-grid">{county_links}</div></div></section>
<section class="section"><div class="container"><div class="callout"><h2>Start a packet for Ohio</h2><p>Check eligibility before proceeding to the $299 checkout.</p><a class="btn btn-secondary" href="#eligibility">Check eligibility</a></div></div></section>''' + build_footer()
    try:
