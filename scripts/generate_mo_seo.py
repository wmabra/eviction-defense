#!/usr/bin/env python3
"""
Generate Missouri SEO pages: state page, all county pages, and 50K+ city pages.
...
COUNTIES = [
    ...
    "Saline", "Schuyler", "Scotland", "Scott", "Shannon", "Shelby", "Stoddard",
    "Stone", "Sullivan", "Taney", "Texas", "Vernon", "Warren", "Washington",
...
CITY_POP = {
    "Kansas City": 516032, "St. Louis": 279695, "Springfield": 170596,
    "Columbia": 130900, "Independence": 121629, "Lee's Summit": 106419,
    "O'Fallon": 95355, "St. Charles": 72458, "St. Joseph": 71098,
    "St. Peters": 61398, "Blue Springs": 61246, "Joplin": 53095,
    "Florissant": 52268,
}
...
def llm(text_prompt):
    ...
    payload = {"model": LLM_MODEL, "messages": [
        {"role": "system", "content": "You are a local writer crafting natural website copy about Missouri locations. No markdown, no em dashes, no asterisks. Plain paragraphs only. Sound like a person."},
...
def content_paragraphs(name, kind):
    if kind == "city":
        prompt = f"Write 2 short paragraphs about {name}, Missouri. P1: what it's known for. P2: what renters should know about the local market and housing help. Return 2 paragraphs separated by a blank line."
    elif kind == "county":
        prompt = f"Write 2 short paragraphs about {name} County, Missouri. P1: what makes it distinct. P2: what a renter should know about rental housing help and the court system. Return 2 paragraphs separated by a blank line."
    else:
        prompt = f"Write 3 short paragraphs about Missouri for a tenant-eviction self-help website. P1: notable fact. P2: rental housing context. P3: practical eviction-process tip. Return 3 paragraphs separated by blank lines."
    t = llm(prompt)
    if not t:
        return [f"{name} is a community in Missouri."] * (3 if kind == "state" else 2)
...
<section class="section"><div class="container"><div class="section-intro"><span class="eyebrow">Statewide starting points</span><h2>Resources that may serve {name} County</h2><p>These statewide or regional portals are starting points, not guarantees of funding or representation.</p></div><div class="resource-list"><article class="resource"><span class="type">Referral</span><h3><a href="https://www.211.org/" rel="nofollow noopener" target="_blank">2-1-1 Missouri ↗</a></h3><p>Rent, utilities, shelter, food and crisis referrals. Dial 211.</p></article><article class="resource"><span class="type">Tenant Help</span><h3><a href="https://motenanthelp.org/" rel="nofollow noopener" target="_blank">Missouri Tenant Help ↗</a></h3><p>Free tenant resource and eviction document engine.</p></article><article class="resource"><span class="type">Legal Aid</span><h3><a href="https://www.lsmo.org/" rel="nofollow noopener" target="_blank">Legal Services of Missouri ↗</a></h3><p>Statewide legal-aid network for low-income tenants.</p></article></div></div></section>
...
def build_city_page(cname, ccounty):
    ...
    head = build_head(
        f"Eviction Help in {cname}, Missouri | evictions.help",
        ...
        f'<li><a href="/">Home</a></li><li><a href="/{STATE_SLUG}/">Missouri</a></li><li><a href="/{STATE_SLUG}/{county_slug}/">{ccounty} County</a></li><li>{cname}</li>',
        f'data-state="MO" data-county="{ccounty} County" data-city="{cname}"'
    )
    hero = build_hero(
...
def main():
    ...
    # 1. State page
    print("Generating Missouri state page...", flush=True)
    paras = content_paragraphs("Missouri", "state")
    content = "\n".join(f"<p>{p}</p>" for p in paras)
    ...
    state_html = build_head(
        "Eviction Help in Missouri | evictions.help",
        "Check eligibility for eviction self-help document preparation across Missouri counties and cities.",
        f"https://evictions.help/{STATE_SLUG}/",
        '<li><a href="/">Home</a></li><li>Missouri</li>',
        'data-state="MO" data-county="" data-city=""'
    ) + build_hero(
        "Missouri eviction document help",
        "Eviction paperwork help for tenants in Missouri",
        "Start with eight questions. Your Missouri location is carried into the guided intake and $299 document packet workflow.",
        'data-state="MO" data-county="" data-city=""'
    ) + f'''<section class="section"><div class="container">{content}</div></section>
<section class="section section-soft"><div class="container"><div class="section-intro"><span class="eyebrow">Counties</span><h2>Missouri county directories</h2></div><div class="location-grid">{county_links}</div></div></section>
<section class="section"><div class="container"><div class="callout"><h2>Start a packet for Missouri</h2><p>Check eligibility before proceeding to the $299 checkout.</p><a class="btn btn-secondary" href="#eligibility">Check eligibility</a></div></div></section>''' + build_footer()
    try:
