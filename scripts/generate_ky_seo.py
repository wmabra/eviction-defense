#!/usr/bin/env python3
"""
Generate Kentucky SEO pages: state page, all county pages, and 50K+ city pages.
...
CITY_POP = {
    "Louisville": 631818, "Lexington": 323725, "Bowling Green": 75388, "Owensboro": 60302,
}
...
def llm(text_prompt):
    ...
    payload = {"model": LLM_MODEL, "messages": [
        {"role": "system", "content": "You are a local writer crafting natural website copy about Kentucky locations. No markdown, no em dashes, no asterisks. Plain paragraphs only. Sound like a person."},
...
def content_paragraphs(name, kind):
    if kind == "city":
        prompt = f"Write 2 short paragraphs about {name}, Kentucky. P1: what it's known for. P2: what renters should know about the local market and housing help. Return 2 paragraphs separated by a blank line."
    elif kind == "county":
        prompt = f"Write 2 short paragraphs about {name} County, Kentucky. P1: what makes it distinct. P2: what a renter should know about rental housing help and the court system. Return 2 paragraphs separated by a blank line."
    else:
        prompt = f"Write 3 short paragraphs about Kentucky for a tenant-eviction self-help website. P1: notable fact. P2: rental housing context. P3: practical eviction-process tip. Return 3 paragraphs separated by blank lines."
    t = llm(prompt)
    if not t:
        return [f"{name} is a community in Kentucky."] * (3 if kind == "state" else 2)
...
<section class="section"><div class="container"><div class="section-intro"><span class="eyebrow">Statewide starting points</span><h2>Resources that may serve {name} County</h2><p>These statewide or regional portals are starting points, not guarantees of funding or representation.</p></div><div class="resource-list"><article class="resource"><span class="type">Referral</span><h3><a href="https://www.211.org/" rel="nofollow noopener" target="_blank">2-1-1 Kentucky ↗</a></h3><p>Rent, utilities, shelter, food and crisis referrals. Dial 211.</p></article><article class="resource"><span class="type">Tenant Help</span><h3><a href="https://motenanthelp.org/" rel="nofollow noopener" target="_blank">Kentucky Tenant Help ↗</a></h3><p>Free tenant resource and eviction document engine.</p></article><article class="resource"><span class="type">Legal Aid</span><h3><a href="https://www.lsmo.org/" rel="nofollow noopener" target="_blank">Legal Services of Kentucky ↗</a></h3><p>Statewide legal-aid network for low-income tenants.</p></article></div></div></section>
...
def build_city_page(cname, ccounty):
    ...
    head = build_head(
        f"Eviction Help in {cname}, Kentucky | evictions.help",
        ...
        f'<li><a href="/">Home</a></li><li><a href="/{STATE_SLUG}/">Kentucky</a></li><li><a href="/{STATE_SLUG}/{county_slug}/">{ccounty} County</a></li><li>{cname}</li>',
        f'data-state="KY" data-county="{ccounty} County" data-city="{cname}"'
    )
    hero = build_hero(
...
def main():
    ...
    # 1. State page
    print("Generating Kentucky state page...", flush=True)
    paras = content_paragraphs("Kentucky", "state")
    content = "\n".join(f"<p>{p}</p>" for p in paras)
    ...
    state_html = build_head(
        "Eviction Help in Kentucky | evictions.help",
        "Check eligibility for eviction self-help document preparation across Kentucky counties and cities.",
        f"https://evictions.help/{STATE_SLUG}/",
        '<li><a href="/">Home</a></li><li>Kentucky</li>',
        'data-state="KY" data-county="" data-city=""'
    ) + build_hero(
        "Kentucky eviction document help",
        "Eviction paperwork help for tenants in Kentucky",
        "Start with eight questions. Your Kentucky location is carried into the guided intake and $299 document packet workflow.",
        'data-state="KY" data-county="" data-city=""'
    ) + f'''<section class="section"><div class="container">{content}</div></section>
<section class="section section-soft"><div class="container"><div class="section-intro"><span class="eyebrow">Counties</span><h2>Kentucky county directories</h2></div><div class="location-grid">{county_links}</div></div></section>
<section class="section"><div class="container"><div class="callout"><h2>Start a packet for Kentucky</h2><p>Check eligibility before proceeding to the $299 checkout.</p><a class="btn btn-secondary" href="#eligibility">Check eligibility</a></div></div></section>''' + build_footer()
    try:
