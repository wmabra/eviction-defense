#!/usr/bin/env python3
"""
Generate Indiana SEO pages: state page, all county pages, and 50K+ city pages.
...
CITY_POP = {
    "Indianapolis": 891484, "Fort Wayne": 273203, "Evansville": 115395, "Fishers": 103986, "South Bend": 103713, "Carmel": 103606, "Bloomington": 79986, "Hammond": 76030, "Noblesville": 75239, "Lafayette": 71757, "Greenwood": 68175, "Gary": 67555, "Muncie": 65320, "Kokomo": 60093, "Terre Haute": 58510, "Westfield": 57746, "Anderson": 55851, "Elkhart": 53690, "Jeffersonville": 52912, "Columbus": 51970, "Mishawaka": 51020,
}
...
def llm(text_prompt):
    ...
    payload = {"model": LLM_MODEL, "messages": [
        {"role": "system", "content": "You are a local writer crafting natural website copy about Indiana locations. No markdown, no em dashes, no asterisks. Plain paragraphs only. Sound like a person."},
...
def content_paragraphs(name, kind):
    if kind == "city":
        prompt = f"Write 2 short paragraphs about {name}, Indiana. P1: what it's known for. P2: what renters should know about the local market and housing help. Return 2 paragraphs separated by a blank line."
    elif kind == "county":
        prompt = f"Write 2 short paragraphs about {name} County, Indiana. P1: what makes it distinct. P2: what a renter should know about rental housing help and the court system. Return 2 paragraphs separated by a blank line."
    else:
        prompt = f"Write 3 short paragraphs about Indiana for a tenant-eviction self-help website. P1: notable fact. P2: rental housing context. P3: practical eviction-process tip. Return 3 paragraphs separated by blank lines."
    t = llm(prompt)
    if not t:
        return [f"{name} is a community in Indiana."] * (3 if kind == "state" else 2)
...
def state_select_html():
    ...
    names = {"AR":"Arkansas","CO":"Colorado","CT":"Connecticut","GA":"Georgia","IL":"Illinois","IN":"Indiana","KY":"Kentucky","LA":"Louisiana","MI":"Michigan","MN":"Minnesota","MO":"Missouri","NM":"New Mexico","OH":"Ohio","OK":"Oklahoma","OR":"Oregon","RI":"Rhode Island","SC":"South Carolina","TN":"Tennessee","TX":"Texas","VA":"Virginia"}
    out = ""
    for s in states:
        sel = ' selected' if s == "IN" else ''
...
<section class="section"><div class="container"><div class="section-intro"><span class="eyebrow">Statewide starting points</span><h2>Resources that may serve {name} County</h2><p>These statewide or regional portals are starting points, not guarantees of funding or representation.</p></div><div class="resource-list"><article class="resource"><span class="type">Referral</span><h3><a href="https://www.211.org/" rel="nofollow noopener" target="_blank">2-1-1 Indiana ↗</a></h3><p>Rent, utilities, shelter, food and crisis referrals. Dial 211.</p></article><article class="resource"><span class="type">Tenant Help</span><h3><a href="https://motenanthelp.org/" rel="nofollow noopener" target="_blank">Indiana Tenant Help ↗</a></h3><p>Free tenant resource and eviction document engine.</p></article><article class="resource"><span class="type">Legal Aid</span><h3><a href="https://www.lsmo.org/" rel="nofollow noopener" target="_blank">Legal Services of Indiana ↗</a></h3><p>Statewide legal-aid network for low-income tenants.</p></article></div></div></section>
<section class="section"><div class="container"><div class="callout"><h2>Start a packet for {name} County</h2><p>Your state and county are prefilled above. Check eligibility before proceeding to the $299 checkout.</p><a class="btn btn-secondary" href="#eligibility">Check eligibility</a></div></div></section>'''
...
def build_city_page(cname, ccounty):
    ...
    head = build_head(
        f"Eviction Help in {cname}, Indiana | evictions.help",
        ...
        f'<li><a href="/">Home</a></li><li><a href="/{STATE_SLUG}/">Indiana</a></li><li><a href="/{STATE_SLUG}/{county_slug}/">{ccounty} County</a></li><li>{cname}</li>',
        f'data-state="IN" data-county="{ccounty} County" data-city="{cname}"'
    )
    hero = build_hero(
        f"{cname} eviction document help",
        f"Prepare eviction paperwork from {cname} with a guided intake",
        "Your city, county, and state are prefilled. Qualified tenants can continue to a $299 self-help document packet built from their own case information.",
        f'data-state="IN" data-county="{ccounty} County" data-city="{cname}"'
    )
    middle = f'''<section class="section"><div class="container">
{content}
</div></section>
<section class="section"><div class="container split"><div><span class="eyebrow">{cname} self-help preparation</span><h2>Your city, county, and state stay connected</h2><p class="lede">{cname} is listed with {ccounty} County in the site's local navigation and had an estimated 2024 population of {pop:,}.</p><p>The page does not claim that evictions are handled by a city-specific court. It carries the city and county into the eligibility and intake flow.</p></div><div class="feature-list"><div class="feature"><i class="check">✓</i><div><strong>Location prefilled</strong><span>{cname}, {ccounty} County, Indiana is preserved in the intake.</span></div></div><div class="feature"><i class="check">✓</i><div><strong>Case-specific questions</strong><span>The intake asks for the landlord, court, case number, dates, amount claimed, payments, defenses, and preferences.</span></div></div><div class="feature"><i class="check">✓</i><div><strong>Conditional documents</strong><span>Letters and motions are included only when the tenant's answers support them.</span></div></div></div></div></section>
<section class="section"><div class="container"><div class="section-intro"><span class="eyebrow">Resource starting points</span><h2>Housing and legal-information portals for Indiana</h2></div><div class="resource-list"><article class="resource"><span class="type">Referral</span><h3><a href="https://www.211.org/" rel="nofollow noopener" target="_blank">2-1-1 Indiana ↗</a></h3><p>Rent, utilities, shelter, food and crisis referrals.</p></article><article class="resource"><span class="type">Tenant Help</span><h3><a href="https://motenanthelp.org/" rel="nofollow noopener" target="_blank">Indiana Tenant Help ↗</a></h3><p>Free tenant resource and eviction document engine.</p></article><article class="resource"><span class="type">Legal Aid</span><h3><a href="https://www.lsmo.org/" rel="nofollow noopener" target="_blank">Legal Services of Indiana ↗</a></h3><p>Statewide legal-aid network.</p></article></div></div></section>
<section class="section section-soft"><div class="container"><div class="section-intro"><span class="eyebrow">Continue exploring</span><h2>Related local pages</h2></div><div class="location-grid"><a class="location-link" href="/{STATE_SLUG}/">All Indiana locations<small>State directory</small></a><a class="location-link" href="/{STATE_SLUG}/{county_slug}/">{ccounty} County<small>County directory</small></a></div></div></section>
...
def main():
    ...
    # 1. State page
    print("Generating Indiana state page...", flush=True)
    paras = content_paragraphs("Indiana", "state")
    content = "\n".join(f"<p>{p}</p>" for p in paras)
    ...
    state_html = build_head(
        "Eviction Help in Indiana | evictions.help",
        "Check eligibility for eviction self-help document preparation across Indiana counties and cities.",
        f"https://evictions.help/{STATE_SLUG}/",
        '<li><a href="/">Home</a></li><li>Indiana</li>',
        'data-state="IN" data-county="" data-city=""'
    ) + build_hero(
        "Indiana eviction document help",
        "Eviction paperwork help for tenants in Indiana",
        "Start with eight questions. Your Indiana location is carried into the guided intake and $299 document packet workflow.",
        'data-state="IN" data-county="" data-city=""'
    ) + f'''<section class="section"><div class="container">{content}</div></section>
<section class="section section-soft"><div class="container"><div class="section-intro"><span class="eyebrow">Counties</span><h2>Indiana county directories</h2></div><div class="location-grid">{county_links}</div></div></section>
<section class="section"><div class="container"><div class="callout"><h2>Start a packet for Indiana</h2><p>Check eligibility before proceeding to the $299 checkout.</p><a class="btn btn-secondary" href="#eligibility">Check eligibility</a></div></div></section>''' + build_footer()
    try:
